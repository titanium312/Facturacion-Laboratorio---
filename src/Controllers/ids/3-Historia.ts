...
export const historia = async (req: Request, res: Response) => {
  try {
    ...
    const response = await axios.get(
      "https://balance.saludplus.co/historiaClinicaUnificada/HIstoriaClinicaBuscar",
      {
        params: { idHistoria },
        responseType: "json",
      }
    ...
    if (data.historia_clinica_procedimientos_diagnosticos) {
      data.historia_clinica_procedimientos_diagnosticos.forEach((proc: any) => {
    ...
    if (data.historia_clinica_procedimientos_terapeuticos) {
      data.historia_clinica_procedimientos_terapeuticos.forEach((proc: any) => {
        todosProcedimientos.push({
          tipo: "terapéutico",
          codigo: proc.fk_procedimiento,
          cantidad: proc.cantidad,
    ...
    if (data.historia_clinica_procedimientos_odontologicos) {
      data.historia_clinica_procedimientos_odontologicos.forEach((proc: any) => {
        todosProcedimientos.push({
          tipo: "odontológico",
          codigo: proc.fk_procedimiento,
          cantidad: proc.cantidad
    ...
    function buscarFkUsuario(obj: any): number | undefined {
